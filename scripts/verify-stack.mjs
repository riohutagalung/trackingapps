...
let ACTIVE_VERCEL = normalizeOrigin(
...
const USE_VERCEL_CLI =
  CLI_MODE !== '0' &&
  (CLI_MODE === '1' || (isVercelUrl(ACTIVE_VERCEL) && !new URL(ACTIVE_VERCEL).hostname.startsWith('rhhabits.vercel.app')));
...
async function listLatestReadyPreview() {
  ...
  for (const line of lines) {
    ...
    if (match) return normalizeOrigin(match[0]);
  }
  ...
  if (useVercelCli) {
    try {
      ...
      if (retryPreview && String(error.message) === 'DEPLOYMENT_NOT_FOUND') {
        ...
        if (latest !== normalizeOrigin(url)) {
          ACTIVE_VERCEL = latest;
          console.log(`\n[RH] Preview lama tidak ditemukan. Beralih ke Preview Ready terbaru: ${ACTIVE_VERCEL}`);
          const replacement = new URL(url);
          replacement.hostname = new URL(ACTIVE_VERCEL).hostname;
          replacement.port = new URL(ACTIVE_VERCEL).port;
          replacement.protocol = new URL(ACTIVE_VERCEL).protocol;
          replacement.pathname = new URL(url).pathname;
          replacement.search = new URL(url).search;
          replacement.hash = '';
...
const page = await request('Vercel HTML', ACTIVE_VERCEL + '/?verify=20260922');
...
await request('Vercel health', ACTIVE_VERCEL + '/api/health');
await request('Vercel GAS health', ACTIVE_VERCEL + '/api/health?probe=gas');
await request('Manifest', ACTIVE_VERCEL + '/manifest.json');
await request('Vercel RPC GET ping', ACTIVE_VERCEL + '/api/rpc?fn=ping&args=' + encodeURIComponent('[]'));
await request(
  'Vercel RPC POST ping',
  ACTIVE_VERCEL + '/api/rpc',
  {
    method: 'POST',
...
await request('Vercel native-location GET', ACTIVE_VERCEL + '/api/native-location');
