...
(function(){
  'use strict';
  const KEY='rh_native_location_disclosure_v2';
  ...
  async function start(id){
    const bg=getPlugin();
    if(!bg) throw new Error('Native GPS belum tersedia. Pastikan app dibuild dengan Capacitor dan npx cap sync sudah dijalankan.');
    sessionId=id;
    const url='https://rhhabits.vercel.app/api/native-location?tripId='+encodeURIComponent(id);
    await bg.start({backgroundMessage:'RH Habits sedang merekam perjalanan. Ketuk untuk kembali ke aplikasi.',backgroundTitle:'RH Habits • Tracking aktif',requestPermissions:true,stale:false,distanceFilter:10,minIntervalMs:5000,url,headers:{'Content-Type':'application/json'}},(location,error)=>{
