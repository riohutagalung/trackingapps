const GAS_URL = 'https://script.google.com/macros/s/AKfycbyBKldRcFJwtLPaCTlkHb7TPOIsdLJnIrhHafHqHwivyoyKAw04oy3jiTlMx_lUwD1gFA/exec';
...
async function postToGas(body){
  ...
  for(let i=0;i<4;i++){
    ...
    if([301,302,303,307,308].includes(r.status)){
      const loc=r.headers.get('location'); if(!loc)throw new Error('Apps Script redirect tidak memiliki Location header.');
      target=new URL(loc,target).toString(); continue;
    }
    const text=await r.text(); let payload=null; try{payload=text?JSON.parse(text):null;}catch(e){}
...
module.exports=async function handler(req,res){
  try{
    ...
    if(!Number.isFinite(lat)||!Number.isFinite(lng)||lat<-90||lat>90||lng<-180||lng>180)return json(res,400,{ok:false,error:'Koordinat GPS tidak valid'});
    const result=await postToGas({...body,tripId,source:'native'});
