...
async function consumeUpstream(res){
  const text=await res.text();let parsed=null;try{parsed=text?JSON.parse(text):null;}catch(e){}
  if(!res.ok){
    let detail='';
  ...
  if(!parsed||typeof parsed!=='object')throw new Error('Apps Script mengembalikan respons bukan JSON. Cek Web App /exec dan deployment yang aktif.');
  return parsed;
}
